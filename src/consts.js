// Centralized constants and data arrays

export const ABOUT_SKILLS = [

  "JavaScript",
  "TypeScript",
  "React",
      "Next.js",
  "Node.js",
  "Express",
  "MongoDB",
  "PostgreSQL",
  "Tailwind CSS",
  "Framer Motion",
  "Git / GitHub",

];

export const PROJECTS = [
  {
    title: "Dr. Ahmed Gad – Fitness Trainer",
    description: "Landing website for a personal gym trainer with strong CTA and Arabic UI.",
    image: "/drahmed.jpg",
    tech: ["React", "Tailwind", "Vite"],
    link: "https://dr-ahmed-gad.com/",
    repo: "https://github.com/HEMA-HEMDAN/drAhmed",
  },
  {
    title: "journey through the egyptain history",
    description: "3D interactions, particles, and smooth scroll built with React and Vite.",
    image: "/journey.jpg",
    tech: ["React", "Vite", "express", "nodejs", "mongodb", "Tailwind"],
    link: "https://masr.pages.dev",
    repo: "/",
  },
  {
    title: "Over Dose Math",
    description: "3D interactions, particles, and smooth scroll built with React and Vite.",
    image: "/math2.jpg",
    tech: ["React", "Vite", "express", "nodejs", "mongodb", "Tailwind"],
    link: "https://overdosemath.com/",
    repo: "/",
  },
  {
    title: "alrehab group",
    description: "website for a nutriton food company with animation , dashboard and admin panel",
    image: "/og.webp",
    tech: ["nextjs", "express", "nodejs", "mongodb", "Tailwind"],
    link: "https://alrehabgp.com/",
    repo: "/",
  },
  {
    title: "wiz academy",
    description: "3D interactions, particles, and smooth scroll built with React and Vite.",
    image: "/wiz.png",
    tech: ["React", "Vite", "express", "nodejs", "mongodb", "Tailwind"],
    link: "https://wiz-9oi.pages.dev/",
    repo: "/",
  },
  {
    title: "alkareem academy",
    description: "3D interactions, particles, and smooth scroll built with React and Vite.",
    image: "/alkareem.png",
    tech: ["React", "Vite", "Tailwind"],
    link: "https://www.alkareemacademy.com/",
    repo: "https://github.com/mohamedwaell/quran",
  },
  {
    title: "elmanasah",
    description: "3D interactions, particles, and smooth scroll built with React and Vite.",
    image: "/elmanasah.jpeg",
    tech: ["React", "Vite", "express", "nodejs", "mongodb", "Tailwind"],
    link: "https://elmanasah.app",
    repo: "/",
  },
  {
    title: "Portfolio Website",
    description: "3D interactions, particles, and smooth scroll built with React and Vite.",
    image: "/portofilo.jpg",
    tech: ["React", "Vite", "Tailwind"],
    link: "/",
    repo: "https://github.com/mohamedwaell/portofilo",
  },
  {
    title: "Tic tac toe js",
    description: "tic tac toe game with javascript",
    image: "/tic.png",
    tech: ["javascript", "html", "css"],
    link: "https://mohamedwaell.github.io/tic-tac-toe-js/",
    repo: "https://github.com/mohamedwaell/tic-tac-toe-js",
  },
];

export const NAV_ITEMS = [
  { label: "Home", href: "#home" },
  { label: "About", href: "#about" },
  { label: "Experience", href: "#experience" },
  { label: "Projects", href: "#projects" },
  { label: "Contact", href: "#contact" },
];

export const PROFILE_IMAGE = "./porfile.jpg";
export const CV_URL = "/Cv.pdf";
export const HERO_TYPING_TEXTS = [
  "I'm a Frontend & Creative Developer.",
  "I build cool things with JS & 3D.",
  "Let's make something awesome!",
];
export const HERO_TEXT_COLORS = ["#fff", "#a47cf3", "#f2d479"];

export const GITHUB_URL = "https://github.com/mohamedwaell";
export const LINKEDIN_URL = "https://www.linkedin.com/in/mohamed-wael-9639bb344/";
export const EMAIL_ADDRESS = "mhamedwaell22@gmail.com";


export const TESTIMONIALS = [
  {
    name: "Dr. Ahmed Gad",
    title: "Fitness Trainer",
    avatar: "/drahmed.jpg",
    quote:
      "والله ي محمد الويب سايت عليه شغل عالي بجد\nالله ينور عليك  \nحاجه جامده مشاء الله",
    link: "https://www.dr-ahmed-gad.com/",
  },
  {
    name: "Mr. Mohamed Hemeda",
    title: "Math Instructor – Over Dose Math",
    avatar: "/math2.jpg",
    quote:
      "Great communication and high-quality work. Our landing page looks modern and performs very well.",
    link: "https://github.com/HEMA-HEMDAN/mo-hemeda",
  },
];

